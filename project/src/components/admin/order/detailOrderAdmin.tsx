    ...
    <>
        ...
        <div className="space-y-6">
          ...
          <div className="dashboard-card p-6">
            ...
            <div className="flex justify-between items-center pt-4 mt-2">
              <span className="text-[#2C1810] font-semibold text-lg">
                Tổng cộng:
              </span>
      ...
      <div className="dashboard-card mb-6 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#6B4E35]/15 bg-[#F5F0E8]/30">
          <h2 className="text-lg text-[#2C1810] font-medium m-0">
            Sản phẩm trong đơn
          </h2>
        ...
        <TableResponsiveWrapper minWidth={780}>
          <table className="w-full text-left border-collapse text-sm text-[#2C1810]">
            <thead className="bg-[rgba(214,161,95,0.15)] text-[#6B4E35] text-xs uppercase tracking-wider">
              <tr>
                <th className="px-6 py-4 font-semibold border-b border-[#6B4E35]/10">
                  Sản phẩm
                </th>
                <th className="px-6 py-4 font-semibold border-b border-[#6B4E35]/10">
                  Mùi hương
                ...
                <th className="px-6 py-4 font-semibold border-b border-[#6B4E35]/10 text-center">
                  SL
                </th>
                <th className="px-6 py-4 font-semibold border-b border-[#6B4E35]/10 text-right">
                  Đơn giá
                </th>
                <th className="px-6 py-4 font-semibold border-b border-[#6B4E35]/10 text-right">
                  Thành tiền
            ...
            <tbody className="divide-y divide-[#6B4E35]/10">
              {order.items.map((item, index) => {
                ...
                return (
                  <tr
                    key={index}
                    className="hover:bg-[#6B4E35]/5 transition-colors"
                  >
                    <td className="px-6 py-4">
                      <div className="font-semibold text-[#2C1810] mb-1">
                        {item.name}
                      </div>
                      {item.detail && (
                        <div className="text-xs text-[#6B4E35]">
                          {item.detail}
                        </div>
                      )}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center font-bold text-[#2C1810]">
                      {item.quantity}
                    </td>
                    <td className="px-6 py-4 text-right text-[#2C1810]">
                      {formatCurrency(item.price)}
                    </td>
                    <td className="px-6 py-4 text-right font-bold text-[#2C1810]">
            ...
            <tfoot>
              <tr className="bg-[#F5F0E8]/30">
                <td
                  colSpan={7}
                  className="px-6 py-4 text-right font-semibold text-[#6B4E35]"
                >
                  Tổng thanh toán
                </td>
                <td className="px-6 py-4 text-right font-bold text-lg text-[#2C1810]">
                  {formatCurrency(order.total)}
                </td>
              </tr>
            </tfoot>
          ...
          <div className="relative border-l border-[#6B4E35]/20 ml-3 space-y-6">
            {order.historyLogs && order.historyLogs.length > 0 ? (
              order.historyLogs.map((log, index) => {
                ...
                const logStatusInfo =
                  statusMap[log.currentStatus] ||
                  ({ label: log.currentStatus, className: "pending" } as const);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-[#6B4E35] mb-1 font-mono">
                      {formatDateTime(log.createdAt)}
                    </div>

                    <div
                      className={`font-semibold ${
                        isLatest ? "text-[#2C1810]" : "text-[#6B4E35]"
                      }`}
                    >
                      {logStatusInfo.label}
                    </div>
                    ...
                    {log.note && (
                      <div className="text-sm text-[#6B4E35] mt-1">
                        {log.note}
                      </div>
                    )}

                    {log.admin && (
                      <div className="text-xs text-[#6B4E35]/70 mt-1">
                        Người thao tác: {log.admin}
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="text-sm text-[#6B4E35] italic py-4">
